...
board = chess.Board()
image = SVG(chess.svg.board(board=board,size=400))
...
try:
  ...
  def alphabeta(alpha, beta, depthleft):
    # Signal for mate in evaluation
    mate = -99999
    # Decreasing depth passed into function
    if depthleft == 0:
      # Check for fair capture
  ...
  def capture(alpha, beta):
    ...
    # For all moves
    for move in board.legal_moves:
      # Internally make a capture
      if board.is_capture(move):
        board.push(move)
      # Take back the capture
  ...
  def select(depth):
    try:
      ...
      # Minimum and maximum alphabeta values
      ...
      for move in board.legal_moves:
        board.push(move)
        # Evaluate the strength of the Move
        value = -alphabeta(-alpha, -beta, depth - 1)
        # If alphabeta search thinks current move is best
        if value > x:
          x = value
          best = move
        elif value > alpha:
  ...
  try:
    ...
    for i in range(length + len(received)):
      print("\b", end="")
    print("Error: You must provide an integer in the range of 1 and 400.")
    sys.exit(1)
